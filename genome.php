...
class Genome extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = 'GENOME';
        $this->description = 'GENOME Payment Module for PrestaShop';
    ...
    public function install()
    {
        ...
        if (!$this->installGenomeOpenOrderState()) {
            return false;
        }
        
        if (!$this->installTab('AdminPayment', 'AdminGenome', 'GENOME')) {
            return false;
        }
    ...
    public function hookActionOrderStatusPostUpdate($params)
    {
        if ($params['newOrderStatus']->id == Configuration::get('PS_OS_REFUND')) {
            ...
            if (Validate::isLoadedObject($order) && ($order->module === 'genome')) {
    ...
    public function hookactionOrderSlipAdd($params)
    {
        ...
        if (isset($params['order']) && ($params['order']->module === 'genome') && Tools::isSubmit('partialRefundProduct')
    ...
    public function getExternalPaymentOption()
    {
        ...
        $externalOption->setCallToActionText($this->l('Genome'))
    ...
    public function uninstall()
    {
        ...
        if (!$this->uninstallTab('AdminGenome')) {
            return false;
        }
    ...
    {
        if (Configuration::get('GENOME_TESTMODE')) {
            $this->publicKey   = Configuration::get('GENOME_PUBLICKEY_TEST');
            $this->secretKey   = Configuration::get('GENOME_SECRETKEY_TEST');
        } else {
            $this->publicKey   = Configuration::get('GENOME_PUBLICKEY');
            $this->secretKey   = Configuration::get('GENOME_SECRETKEY');
        }
    }

    private function deleteModuleSettings()
    {
        Configuration::deleteByName('GENOME_PUBLICKEY');
        Configuration::deleteByName('GENOME_SECRETKEY');
        Configuration::deleteByName('GENOME_PUBLICKEY_TEST');
        Configuration::deleteByName('GENOME_SECRETKEY_TEST');
        Configuration::deleteByName('GENOME_TESTMODE');
    }
    ...
    private function installGenomeOpenOrderState()
    {
        if (Configuration::get('PS_OS_GENOME_OPEN') < 1) {
            $order_state = new OrderState();
            $order_state->name = array();
            
            foreach (Language::getLanguages(false) as $language) {
                $order_state->name[(int)$language['id_lang']] = $this->l('Awaiting Genome payment');
            }
            ...
            if ($order_state->add()) {
                Configuration::updateValue("PS_OS_GENOME_OPEN", $order_state->id);
            } else {
                return false;
            }
        }
